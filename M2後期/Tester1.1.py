...
try:
    config = load_config('./config/middleware_config.yaml')
...
tester_cfg = config.get('Tester', {}) or {}
try:
    peer_nums = int(tester_cfg.get('peer_nums', tester_cfg.get('n_down', peer_nums)))
except Exception:
    peer_nums = peer_nums

try:
    total_rounds = int(tester_cfg.get('total_rounds', tester_cfg.get('n_msg', total_rounds)))
except Exception:
...
DOWN_IDS = [f"Down{i}".encode('utf-8') for i in range(0, peer_nums)]
...
class TesterRunner:
        ...
        try:
            while not self.stop_event.is_set():
                if sock.poll(100, zmq.POLLIN):
                    ...
                    logger.info(f"Downstream {down_id.decode()} received: {msg} latency={latency:.3f}ms")
        except zmq.ZMQError as e:
    ...
    def print_performance_report(self):
            ...
            for down_id in DOWN_IDS:
                ...
                for i in range(total_rounds):
                    ...
                    if latencies:
                        ...
                        logger.info(
                            f"  Data {i}: Latency={avg_latency:.3f}ms Count={len(latencies)}")
        ...
        logger.info("\n------ Summary ------")
        if all_latencies:
            logger.info(f" Overall average latency={sum(all_latencies) / len(all_latencies):.3f}ms")
            logger.info(f" Max={max(all_latencies):.3f}ms, Min={min(all_latencies):.3f}ms")
